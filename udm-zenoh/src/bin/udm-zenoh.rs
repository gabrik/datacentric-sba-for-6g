...
use udm_zenoh::server::Server;
use udm_zenoh::UDMApi;
...
async fn main() {
    ...
    config
        .set_listen(
            ListenConfig::new(vec![EndPoint {
                locator: Locator::from_str("tcp/127.0.0.1:7071").unwrap(),
                config: None,
            }])
            .unwrap(),
        )
    ...
    let api_server = server.get_udm_api_server(session, None);
