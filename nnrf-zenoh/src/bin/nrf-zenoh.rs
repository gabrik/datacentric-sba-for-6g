use nnrf_zenoh::server::Server;
use nnrf_zenoh::NRFApi;
...
async fn main() {
    ...
    config
        .set_listen(
            ListenConfig::new(vec![EndPoint {
                locator: Locator::from_str("tcp/127.0.0.1:7070").unwrap(),
                config: None,
            }])
            .unwrap(),
        )
    ...
    let api_server = server.get_nrf_api_server(session, None);
